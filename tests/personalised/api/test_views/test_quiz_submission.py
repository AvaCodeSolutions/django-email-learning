...
def test_quiz_submission_api_valid_token(content_delivery, anonymous_client):
    ...
    response = anonymous_client.post(
        ...
        data={
            "token": token,
            "answers": [{"id": q.id, "answers": []} for q in content_delivery.course_content.quiz.questions.all()],
        },
    ...
    assert response.json()["required_score"] == content_delivery.course_content.quiz.required_score
...
def test_calculate_score_and_passed_static_method(quiz_with_questions):
    ...
    score, passed = QuizSubmissionView.calculate_score_and_passed(quiz_with_questions, answers, question_ids=None)
...
def test_amp_quiz_submission_invalid_token_returns_400_with_amp_headers(
    ...
    response = anonymous_client.post(
        ...
        headers={"Origin": REQUEST_ORIGIN},  # Set origin to a trusted origin to bypass origin check
    )
...
def test_amp_quiz_submission_expired_token_returns_410_with_amp_headers(content_delivery, anonymous_client):
    token = jwt_service.generate_jwt(
        {
            "delivery_id": content_delivery.id,
            "delivery_hash": content_delivery.hash_value,
...
def test_amp_quiz_submission_success_returns_200_with_amp_headers(content_delivery, anonymous_client):
    token = jwt_service.generate_jwt(
        {
            "delivery_id": content_delivery.id,
            "delivery_hash": content_delivery.hash_value,
