...
URL = reverse("django_email_learning:api_jobs:deactivate_inactive_enrollments")


def test_deactivate_inactive_enrollments_without_api_key(superadmin_client):
...
def test_deactivate_inactive_enrollments_valid_jwt_format_but_invalid_api_key(
    superadmin_client,
):
...
def test_deactivate_inactive_enrollments_with_valid_api_key(mock_submit, superadmin_client):
    ...
    body = response.json()
    assert body["status"] == "DeactivateInactiveEnrollmentsJob triggered"
    job_execution = JobExecution.objects.get(id=body["job_execution_id"])
    assert job_execution.job_name == JobName.DEACTIVATE_ENROLLMENTS.value
...
def test_deactivate_inactive_enrollments_already_running_returns_409(superadmin_client):
    ...
    assert response.json() == {
        "status": "DeactivateInactiveEnrollmentsJob already running",
...
def test_deactivate_inactive_enrollments_submission_failure_triggers_job_execution_failed_metric(
    ...
    job_execution = JobExecution.objects.get(job_name=JobName.DEACTIVATE_ENROLLMENTS.value)
