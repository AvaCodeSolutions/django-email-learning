...
def test_send_reminders_with_valid_api_key(mock_submit, superadmin_client):
    ...
    body = response.json()
    assert body["status"] == "SendRemidersJob triggered"
    job_execution = JobExecution.objects.get(id=body["job_execution_id"])
    assert job_execution.job_name == JobName.SEND_REMINDERS.value
...
def test_send_reminders_already_running_returns_409(superadmin_client):
    ...
    assert response.json() == {
        "status": "SendRemidersJob already running",
...
def test_send_reminders_submission_failure_triggers_job_execution_failed_metric(
    ...
    job_execution = JobExecution.objects.get(job_name=JobName.SEND_REMINDERS.value)
