...
@mock.patch(
    "django_email_learning.jobs.send_newsletters_job.SendNewslettersJob.run",
    return_value=None,
)
def test_send_newsletters_with_valid_api_key(mock_run, superadmin_client):
    ...
    assert response.json() == {"status": "SendNewslettersJob triggered"}
...
def test_send_newsletters_failed_triggers_job_execution_failed_metric(
    ...
    mock_job_execution_failed.assert_called_once_with(job_name="send_newsletters")
