...
- **Branching courses: learners can be routed down separate tracks** — by their quiz result, or by the answer they pick on a new decision point. A course without routing rules behaves exactly as before.
  - **Tracks** (`ContentTrack`) are named runs of content beside the main path. A track branches off the main path or another track, nested up to ten levels deep, and rejoins the course at a chosen content — only ever outward from where it sits — or ends the course. **Add Track** creates one, each track's header in the content table edits or deletes it (a track still holding content cannot be deleted), and **Move to** or the new **Track** field in the content forms places content on it.
  - **Routing rules** (`ContentTransition`) on a quiz or a decision point name a condition — `passed`, `failed`, `score_gte`, `score_lt`, `option_selected` or `default` — and a target track. They are checked in order and the first match wins; an `otherwise` rule must come last. The **Branching** tab of a quiz or decision point edits them, saved as a whole through `PUT …/contents/<content_id>/transitions/`. Rules only route by a result: when there is none — a deadline passing, unpublished content — the learner continues in order. Any rule, move, reorder or track edit that would rejoin a track at or before its branch point is refused.
  - **A quiz with routing rules routes on its first submission**, with no retry, no attempt limit and no two-strikes deactivation: failing is a destination rather than the end of the enrollment.
  - **Decision points** — the `decision` content type, backed by `DecisionPoint`, `DecisionOption` and `DecisionResponse` — ask one question with two or more answers and no right answer. They are sent like a quiz, with deadlines and reminders; the first answer is final, retires the link and routes the learner, and with `AMP_ENABLED` the email can be answered in place. A decision point is never blocking, so a missed deadline moves the learner on. **Add Decision** opens its form, and an answer a learner has already picked cannot be removed.
  - **Flow view** on the Content tab — a Table / Flow toggle, drawn with the new `@xyflow/react` frontend dependency — shows a branching course as a diagram: each track as a box, and every move a learner can make as an arrow labelled with its rule. Unpublished content is faded, and the routes it cannot take are dotted.
  - **Path taken** in the enrollment dialog on the Learners page, and a **Tracks** table on the course Analytics tab — routed onto it, finished, still on it, left the course — served by the new `GET /api/analytics/organizations/<organization_id>/track-breakdown/?course_id=<course_id>`.
  - **API**: `…/courses/<course_id>/tracks/` and `…/contents/<content_id>/transitions/` author tracks and rules, readable by every organization role and writable by admins and editors. The contents listing carries each content's `track_id` plus the course's `tracks` and `transitions`, and `GET …/enrollments/<enrollment_id>/` carries `path` and `has_branching`.

### Changed

- **Progress on a branching course is reported two ways.** `Enrollment.learner_progress_percentage()`, what learners are shown, is `None` for every learner on a course with any routing rule: no single total is right for everyone once a course routes anyone. `Enrollment.progress_percentage()`, what the platform reports, is measured against the learner's own route — what they have been sent plus what is still ahead of them — so it reaches 100% on a route that skips content, and can go down when a learner is routed onto a longer track. `bulk_progress_percentages()` still runs a fixed number of queries however many enrollments it covers.
...
- `0028_contenttransition` creates `ContentTransition`.
- `0029_decisionpoint` creates `DecisionPoint`, `DecisionOption` and `DecisionResponse`, and adds the nullable `CourseContent.decision` and `ContentTransition.option`.

All three are additive and reversible, with no data migration.
