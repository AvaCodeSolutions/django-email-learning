...
- **A map of a branching course, the route each learner took, and how learners fared on each track** — the read-only side of branching.
  - **Map view on the Content tab**, offered once a course has tracks: the main path top to bottom and, at each branch point, the tracks its rules route onto side by side, each ending where it rejoins or noting that it ends the course. A branch point without an `otherwise` rule says that a learner no rule claims continues on the path. Clicking a node opens the content, as a table row does. The table stays mounted underneath and remains the authoring surface.
