...
- **`Enrollment.has_branched()`**, and `progress_percentage()` now returns `None` for an enrollment that has been routed onto a track. The denominator is the course's published content, which is the right total only for a learner walking the main spine: a branched learner is on a path whose length depends on the route they took, so any percentage against the whole course either understates them or counts content they will never be sent. A learner who never branches — every learner in a course with no rules — still reports a number.
  - Consumers render nothing rather than a wrong number: the lesson email omits its progress bar, the learners API and `EnrollmentSummaryResponse.progress` carry `null`, the progress CSV has a blank cell, and average progress measures the learners it can measure while still counting the rest in `active_enrollments`.
  - `bulk_progress_percentages()` stays at a constant query count, now three rather than two.
...
- **`ContentTrack`, the structure conditional routing will route onto** — a named branch of a course, holding a run of content ordered among itself rather than against the main spine. `CourseContent.track` places content on one; empty, which is every row today, means the main spine. A track continues at its `merge_into` when it runs out, and `parent_track` lets a track branch again, with the walk climbing it for the first merge point a nested track offers.
  ...
  - **A merge may only move outward** — onto the main spine, or onto a track this one branches off, never a sibling or a track nested under it. Nesting depth then strictly decreases each time a track runs out, so the walk terminates by construction rather than by its loop guard. The remaining half of forward-only — that a merge lands *after* the content the learner branched at — needs `ContentTransition` to say where that was, and lands with routing.
  ...
  - Track nesting is capped at `MAX_NESTING_DEPTH` and the cap is enforced when a track is saved, not merely used to bound a traversal. Detecting a parent cycle by walking a fixed number of levels is unsound — a chain nested past the bound could be closed into a cycle the check never walked far enough to meet — so `ancestors()` stops when the chain repeats instead, and the depth limit is a separate authoring rule.
