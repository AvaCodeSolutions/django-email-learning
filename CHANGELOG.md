...
- **Decision points: a one-question choice that routes learners by the answer they pick** — a new `decision` content type. A decision point asks one question with two or more answers and has no right answer; the answer a learner picks is what its routing rules match.
  - **Sent like a quiz.** The email links to a page showing the question and its answers, and nothing after it is scheduled until the learner answers. With `AMP_ENABLED`, it also carries an AMP version in which the learner answers inside the email, through the new `…/decisions/amp/` endpoint; it checks the sender and origin exactly as the quiz form does, and shows why an answer was refused — already answered, say — with a link to the page instead. Deadlines and reminders work as they do for quizzes, but a decision point is never blocking: a missed deadline moves the learner on in plain order rather than deactivating the enrollment, since there is no wrong answer to fail on.
  - **The first answer is the only one.** The new `…/decisions/` submission endpoint records a `DecisionResponse`, retires the link and routes the learner. A decision point without rules records the answer and continues to the next content. Opening a link that was already answered says so.
  - **Answer rules.** `ContentTransition` gains the `option_selected` condition and an `option` naming the answer it matches, which must belong to the source's decision point. A decision point accepts only answer rules and `otherwise`, and quiz conditions stay quiz-only. `PUT …/contents/<content_id>/transitions/` takes an `option_id` per rule, and transitions in the API now carry `option_id` and `option_text`. With no answer to route by — a deadline passing — the rules are ignored and the learner walks on in order, as with quizzes.
  - **Authoring.** **Add Decision** opens a form for the question, its answers (added, removed and reordered, at least two), the send delay, a deadline or reminders, and the track. Once saved, its **Branching** tab edits the rules with an answer picker in place of a score. The content table and the flow view label an answer rule with its answer (`If answered "Deeper dive"`). `POST …/contents/` takes `{"type": "decision", …, "options": [{"text": …}]}`; an update takes the full list of answers in order — answers with an `id` are edited, those without are added, and those left out are removed along with the rules on them. Removing an answer a learner has already chosen is refused with a 409.
  - Progress is unchanged: a course whose decision points carry rules branches like any other, so learners are shown no percentage.

- **A map of a branching course, the route each learner took, and how learners fared on each track** — the read-only side of branching.
  - **Flow view on the Content tab** (a Table / Flow toggle), offered once a course has tracks, drawn with React Flow (`@xyflow/react`), a new frontend dependency. Every content is a node and each track is a tinted box around its content, nested inside the box of the track it branches off — so anything outside every box is on the main path. The main path runs straight down its own column and each track gets a band of columns beside it, sharing a column with another track only when a free row separates them, so track boxes never overlap each other or the main path; the gap between columns grows with nesting so nested boxes always fit. An arrow leaving a track turns towards where it continues just above that content, so however many rows it crosses it never looks attached to the content before it. Every move a learner can make is an arrow: down the path, onto a track with the rule that selects it (`If failed`, `If score ≥ 80`), and back to where a finished track rejoins, or on to the end of the course. A branch point's plain next step is drawn too — labelled `Otherwise` when no rule catches every result, or dashed and labelled `If not submitted` behind an `otherwise` rule, since rules are ignored when there is no result to route by. Unpublished content is faded to 70% opacity, and the routes out of an unpublished branch point are dotted, since content that is never sent routes no one — its plain next step is drawn as the path everyone takes. Content on a track no rule reaches is flagged, and clicking a node opens the content as a table row does; for admins and editors, clicking a track's name opens the track for editing. The flow view is as tall as the course needs to be read at 100% — never shorter than its base height and at most twice it, past which it zooms out to fit — and it never zooms in beyond 100%. The table stays mounted underneath and remains the authoring surface. Reordering or publishing content in the table shows in the flow view without a reload.
  - **Path taken** in the enrollment dialog on the Learners page: every content the learner has been sent or is scheduled to receive, in order, grouped by track and marked sent, scheduled or not sent. `GET …/enrollments/<enrollment_id>/` now carries `path` and `has_branching`.
  - **A Tracks table on the course's Analytics tab**, from the new `GET /api/analytics/organizations/<organization_id>/track-breakdown/?course_id=<course_id>`. A learner was routed onto a track once they were sent content on it; they finished it once they were then sent content anywhere outside it or completed the course, left the course if their enrollment was deactivated first, and are otherwise still on it. Content on a track nested inside counts as the same track, since branching again is still being on it.
  - `buildRouteTree()` now holds the only traversal of a course's branching, and the content table's `buildContentTree()` flattens it — so the table and the flow view cannot disagree about where a track sits.

- **Authoring UI for branching** — tracks and routing rules can now be built from the course page instead of only over the API.
  - **The content table shows the course's shape.** Each track sits directly under the content that routes onto it, opened by a header naming the rule (`If failed`, `If score ≥ 80`) and closed by a row naming where the learner rejoins, or that the track ends the course. A track reached from several quizzes appears once, noting the others. Tracks no rule reaches yet are listed at the bottom so they stay editable. `GET …/courses/<course_id>/contents/` now carries `track_id` on each content plus the course's `tracks` and `transitions`, so the table renders from one request.
  - **Add Track** creates a track, choosing what it branches off and where it rejoins; the header of each track edits or deletes it. The rejoin choices are limited to the main path and the tracks it branches off, the only merge points the model accepts. `POST`/`DELETE …/courses/<course_id>/tracks/<track_id>/` back this, and deleting a track that still holds content is refused with a 409.
  - **Move to** on a content row puts it at the end of another track or back on the main path, through a new `track_id` on the ordinary content update — present-but-null means the main path.
  - **The lesson, quiz and assignment forms choose a track.** Creating content with a track numbers it after the last content on that track, and changing the track on existing content moves it to the end of the new one under the same routing checks as Move to. `POST …/courses/<course_id>/contents/` accepts `track_id`, refusing a track from another course with a 404. The choice is hidden on a course with no tracks.
  - **A Branching tab on the quiz dialog** edits its routing rules as a list: condition, score threshold for score rules, target track, and up/down to set which rule matches first. The set is saved whole through `PUT …/contents/<content_id>/transitions/`, which replaces the rules in one transaction and refuses an `otherwise` rule anywhere but last, since nothing after it could match. On a quiz that branches, **limited attempts** is disabled with an explanation, because the first submission decides the route and there is no retry. The tab fetches nothing until it is opened.

- **Conditional routing: a quiz result can send a learner down a different path** — `ContentTransition` attaches routing rules to content. Each names a condition (`passed`, `failed`, `score_gte`, `score_lt`, `default`), an optional threshold and a target `ContentTrack`; the rules on a content are evaluated in `order` and the first match wins. Content carrying no rules is walked in plain priority order, so a course that does not branch is unaffected.
  - **A quiz with routing rules answers with a route, not a verdict.** The first submission sends the learner wherever their result points, and the retry, the attempt limit and the two-strikes deactivation never come into play — failing is a destination rather than the end of the enrollment. The existing cascade is untouched for every quiz without rules, so it is bypassed rather than edited.
  - `content_sequence_service.next_content()` takes the outcome the content produced. Without one — a lesson going out, a deadline passing, the inactivity job stepping an enrollment along — only the linear walk applies, so nothing routes a learner behind their own back.
  - Validation refuses a target track that rejoins the course at or before its branch point, which would route the learner onto it again, and again. This is the half of forward-only that `ContentTrack` alone could not check: a track has no branch point until a rule names one.
  - `POST`/`GET` `…/courses/<course_id>/tracks/` and `…/courses/<course_id>/contents/<content_id>/transitions/` author both, readable by every organization role and writable by admin and editor. There is no authoring UI yet.

- **Progress is reported twice, because the learner and the platform are asking different questions.**
  - `Enrollment.learner_progress_percentage()` is what the learner is shown, and it is `None` for **every** learner on a course that branches at all — decided by the course, not by the enrollment. A percentage only means "how much of this course is behind you" while everyone walks the same content; once a course routes anyone, no total is right for everyone, including the learners who happen to take the straight line through it. Deciding it per enrollment instead would make the progress bar *vanish* halfway through a branched learner's emails, which reads as a bug, where its absence throughout reads as a course that does not track progress. A course holding a track nothing routes onto is still a straight line, so `Course.has_branching()` asks about transitions rather than tracks.
  - `Enrollment.progress_percentage()` is what the platform reports, and stays an `int`. Its total is what this learner has been sent plus what is still ahead, projected from where they stand on the assumption that they do not branch again — **not** the whole course, which would charge them for the content their route skipped and leave a learner who had finished reporting 75%, or less still off a track that ends the course early.
  - It moves when a learner is routed, because their path genuinely changes length: down onto a longer remedial track, up through a shortcut past content they no longer need. That is the reason the two numbers are not one: "56%, down from 71% when they were routed onto remediation" answers an operator's question and would only demoralise the person sitting the course. Two learners on one course are measured against different totals, which is worth knowing when reading an average.
  - `bulk_progress_percentages()` still does not scale with enrollment count. Every learner needs their own walk, so `CoursePath` loads the course's shape once and walks it in memory: four queries whether or not anyone has branched, and CPU over a few dozen contents rather than a query each.

### Changed

- **Reordering course content is per track, and cannot break routing.** Priorities are ordered within a track, so the reorder endpoint refuses a list spanning two tracks. After renumbering it re-checks every track and rule in the course through the new `Course.validate_branching()` and rolls back with a 409 if the new order would put a merge point at or before its branch point — a shape that would route learners onto the same track again. Moving content between tracks and editing a track's merge point run the same check. The content table now sends the new order once, on drop, rather than on every row it passes while dragging: a position crossed mid-drag can be exactly such a refused shape.
...
- `0029_decisionpoint` creates `DecisionPoint`, `DecisionOption` and `DecisionResponse`, adds the nullable `CourseContent.decision` with the `unique_decision_per_course` constraint, and adds the nullable `ContentTransition.option`. The `decision` content type and the `option_selected` condition only widen existing choices. Additive and reversible, with no data migration.
- `0028_contenttransition` creates `ContentTransition`. Additive and reversible, with no data migration: a course with no rules keeps its existing behaviour exactly.
