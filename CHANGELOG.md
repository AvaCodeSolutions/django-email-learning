...
- **Decision points: a one-question choice that routes learners by the answer they pick** — a new `decision` content type. A decision point asks one question with two or more answers and has no right answer; the answer a learner picks is what its routing rules match.
  ...
  - **Authoring.** **Add Decision** opens a form for the question, its answers (added, removed and reordered, at least two), the send delay, a deadline or reminders, and the track. Once saved, its **Branching** tab edits the rules with an answer picker in place of a score. The content table and the course map label an answer rule with its answer (`If answered "Deeper dive"`). `POST …/contents/` takes `{"type": "decision", …, "options": [{"text": …}]}`; an update takes the full list of answers in order — answers with an `id` are edited, those without are added, and those left out are removed along with the rules on them. Removing an answer a learner has already chosen is refused with a 409.
...
- **A map of a branching course, the route each learner took, and how learners fared on each track** — the read-only side of branching.
  ...
  - `buildRouteTree()` now holds the only traversal of a course's branching, and the content table's `buildContentTree()` flattens it — so the table and the map cannot disagree about where a track sits.
