...
- **Progress is reported twice, because the learner and the platform are asking different questions.**
  ...
  - `Enrollment.progress_percentage()` is what the platform reports, and stays an `int`. Its total is the main spine plus the tracks the enrollment has actually entered, so it **drops when a learner is routed** — content they had not been sent has just been added to what they owe. That is a real thing for an operator to know and the reason the two numbers are not the same one: "56%, down from 71% when they were routed onto remediation" answers their question and would only demoralise the person sitting the course. Two learners on one course can therefore be measured against different totals, which is worth knowing when reading an average.
  - `bulk_progress_percentages()` still does not scale with enrollment count: three queries, or four once any enrollment has branched.
