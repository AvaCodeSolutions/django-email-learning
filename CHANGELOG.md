...
- **Progress is reported twice, because the learner and the platform are asking different questions.**
  ...
  - `bulk_progress_percentages()` still does not scale with enrollment count. Every learner needs their own walk, so `CourseGraph` loads the course's shape once and walks it in memory: four queries whether or not anyone has branched, and CPU over a few dozen contents rather than a query each.
