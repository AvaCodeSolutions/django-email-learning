...
class SendAssignmentCommand(AbstractCommand):
    ...
    def execute(self) -> None:
        ...
        if not content.assignment:
            raise AssignmentNotFoundError(f"CourseContent with ID {self.content_id} has no associated assignment")
        self.logger.info(f"Sending assignment with ID {content.assignment.id} to email {mask_email(self.email)}")
        ...
        email_message.attach_alternative(render_to_string("emails/assignment.html", context), "text/html")
