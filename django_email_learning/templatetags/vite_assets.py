...
def vite_asset_tags(entry: str) -> str:
    ...
    for css_url in css_urls:
        # URLs from manifest are trusted (generated by Vite), but escape them for safety
        safe_url = quote(css_url, safe='/:.-_')
        tags.append(f'<link rel="stylesheet" crossorigin href="{settings.STATIC_URL}{safe_url}">')
    
    # Add script tag for main entry
    for script_url in script_urls:
        safe_url = quote(script_url, safe='/:.-_')
        tags.append(f'<script type="module" crossorigin src="{settings.STATIC_URL}{safe_url}"></script>')
    ...
    if entry in manifest and "imports" in manifest[entry]:
        for import_key in manifest[entry]["imports"]:
            if import_key in manifest:
                ...
                safe_url = quote(import_file, safe='/:.-_')
                tags.append(f'<link rel="modulepreload" crossorigin href="{settings.STATIC_URL}{safe_url}">')
