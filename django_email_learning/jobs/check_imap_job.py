...
class CheckIMAPJob:
    ...
    def _get_imap_interface(self) -> ImapInterfaceProtocol:
        ...
        try:
            imap_interface = import_string(DJANGO_EMAIL_LEARNING_SETTINGS["IMAP_INTERFACE"])
            return imap_interface() if isinstance(imap_interface, type) else imap_interface
        except KeyError:
    ...
    def start(self) -> JobExecution | None:
        job_execution = JobExecution.start_if_not_running(job_name=JobName.CHECK_IMAP.value)
        if job_execution is None:
            logger.warning("Another instance of CheckIMAPJob is already running. Exiting this run.")
    ...
    def _run_job(self, job_execution: JobExecution) -> None:
        ...
        for imap_connection in imap_connections:
            ...
            logger.info(f"Checking account {account} for IMAP connection {imap_connection.id}")
            if not account:
                continue

            try:
                folders = list(imap_connection.folders.all().values_list("folder_name", flat=True))
    ...
    def _connect_account(self, imap_connection: ImapConnection) -> imaplib.IMAP4_SSL | None:
        try:
            ...
            logger.error(f"Failed to connect/login to IMAP server for connection {imap_connection.id}: {str(e)}")
            return None

    def _process_folder(self, account: imaplib.IMAP4_SSL, folder: str, imap_connection: ImapConnection) -> None:
        try:
        ...
        if result != "OK":
            logger.warning(f"Failed to search unseen emails in folder {folder} for connection {imap_connection.id}")
            return

        mail_ids = data[0].split()
        if not mail_ids:
            logger.info(f"No new emails in folder {folder} for connection {imap_connection.id}.")
            return

        logger.info(f"Found {len(mail_ids)} new emails in folder {folder} for connection {imap_connection.id}.")
    ...
    def _process_email(
        ...
        if result != "OK":
            logger.warning(f"Failed to fetch email with ID {email_id} for connection {imap_connection.id}")
            return
    ...
    def _logout_account(self, account: imaplib.IMAP4_SSL, imap_connection: ImapConnection) -> None:
        try:
            account.logout()
        except Exception as e:
            logger.warning(f"Failed to logout from IMAP server for connection {imap_connection.id}: {str(e)}")
