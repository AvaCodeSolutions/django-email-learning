...
import { Alert,Box, Button, FormControlLabel, Grid, InputLabel, MenuItem, Select, Switch, Tab, Tabs, Tooltip, Typography, Dialog, LinearProgress } from '@mui/material';
...
const QuizForm = ({cancelCallback, successCallback, courseId, quizId, contentId, initialRequiredScore, initialTitle, initialQuestions, initialWaitingPeriod, initialStrategy, initialDeadlineDays, initialLimitedAttempts, initialIsBlocking, initialReminderIntervalDays }) => {
    ...
    return (
         <Box ref={dialogRef} sx={{ px: { xs: '14px', sm: 3 }, py: 3 }} tabIndex={0} focusable="true" onKeyDown={(e) => {
            ...
            {/* Kept mounted rather than unmounted so switching to Analytics and back does not
                discard unsaved authoring edits; display:none also drops it from the a11y tree. */}
            <Box role="tabpanel" sx={{ display: activeTab === 'questions' ? 'block' : 'none' }}>
